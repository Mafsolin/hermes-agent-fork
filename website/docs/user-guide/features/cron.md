...
- **Global default** — only when neither of the above is set does a job follow `hermes model`. In this Afsol downstream, that follow-at-fire-time behavior is used when `cron.model_drift_guard: false` is set; creation-time values are then retained as metadata and do not block a run after a global model switch. Pin the provider/model explicitly when a job must stay on a particular paid route (`hermes cron edit <job_id> --provider <provider> --model <model>`). Setting `cron.model` (or a per-job pin) remains the clearest way to route cron spend independently from interactive chat.
